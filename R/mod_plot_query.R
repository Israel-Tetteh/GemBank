...
mod_plot_query_ui <- function(id) {
  ...
  tagList(
    div(
      ...
      p(
        class = "text-muted",
        "Search and filter all field plots across all trials."
      )
    ),
     ...
     bslib::card(
      ...
      bslib::card_body(
        ...
        bslib::layout_columns(
          ...
          selectizeInput(ns("filter_accession"), "Accession Name", choices = NULL),
...
#' @noRd
mod_plot_query_server <- function(id, db_state, global_refresh_trigger) {
        ...
        tryCatch(
          {
            ...
            updateSelectizeInput(
              session,
              "filter_rep",
              choices = c("All", get_all_replications(db_state$path)),
              server = TRUE
            )
          },
          error = function(e) {
